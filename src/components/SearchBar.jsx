...
const allActions = [
  { id: "1", label: "Book tickets", icon: <PlaneTakeoff size={16} />, description: "Operator", short: "⌘K", end: "Agent" },
  { id: "2", label: "Summarize", icon: <BarChart2 size={16} />, description: "gpt-4o", short: "⌘cmd+p", end: "Command" },
  { id: "3", label: "Screen Studio", icon: <Video size={16} />, description: "gpt-4o", end: "Application" },
  { id: "4", label: "Talk to Jarvis", icon: <AudioLines size={16} />, description: "gpt-4o voice", end: "Active" },
  { id: "5", label: "Translate", icon: <Globe size={16} />, description: "gpt-4o", end: "Command" },
];
...
const SearchBar = () => {
  ...
  const buscarDeputados = async () => {
    ...
    try {
      ...
      else if (searchType === "sigla_partido") params.append("sigla_partido", query);

      const res = await fetch(`http://localhost:8000/deputados/?${params.toString()}`);
      if (!res.ok) throw new Error("Erro ao buscar deputados");
      const data = await res.json();

      const dadosFormatados = Array.isArray(data.dados) ? data.dados : Array.isArray(data) ? data : [];
  ...
  return (
    <div className="searchbar-container">
      ...
      {selectedItem && searchType !== "comandos" && (
        <motion.div className="deputado-detalhes" initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }}>
          ...
          {selectedItem.uri && (
            <p>
              <a href={selectedItem.uri} target="_blank" rel="noopener noreferrer" className="link-perfil">
